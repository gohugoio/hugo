...
func (h *HugoSites) processPartial(ctx context.Context, l logg.LevelLogger, config *BuildCfg, init func(config *BuildCfg) error, events []fsnotify.Event) error {
		...
		case files.ComponentFolderLayouts:
			...
			if tmplAdded {
				...
				if strings.Contains(base, "_markup") {
					...
					identities = append(identities, identity.GenghisKhan)
				}
	...
	for _, id := range changedPaths.deleted {
		handleChange(id, true, false)
	}
	...
	// Removes duplicates.
	identities = changed.identitySet.AsSlice()

	if err := h.resolveAndClearStateForIdentities(ctx, l, cacheBusterOr, identities...); err != nil {
		return err
	}
