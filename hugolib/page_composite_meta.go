...
func (pm *pageMeta) setMetadata(p *pageState, frontmatter map[string]interface{}) error {
	...
	for k, v := range frontmatter {
		...
		case "slug":
			pm.urlPaths.Slug = cast.ToString(v)
		...
		case "sitemap":
			p.m.sitemap = config.DecodeSitemap(p.s.siteConfigHolder.sitemap, cast.ToStringMap(v))
	...
	if !sitemapSet {
		pm.sitemap = p.s.siteConfigHolder.sitemap
	}
