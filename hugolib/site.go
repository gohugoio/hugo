...
type Site struct {
	...
	*siteVersionRole
	...
	siteRefLinker      siteRefLinker // TODO1 clone or something.
	publisher          publisher.Publisher
	frontmatterHandler pagemeta.FrontMatterHandler
}

func (s Site) cloneForVersionAndRole(version, role int) (*Site, error) {
	s.siteVersionRole = s.siteVersionRole.cloneForVersionAndRole(version, role)
...
func (s siteVersionRole) cloneForVersionAndRole(version, role int) *siteVersionRole {
	s.dims[sitematrix.Version.Index()] = version
	s.dims[sitematrix.Role.Index()] = role
	s.home = nil
	s.lastmod = time.Time{}
	s.init = &siteInit{}
	s.taxonomies = nil
...
func (s siteVersionRole) Role() roles.Role {
	return s.role
}

// TODO1 remove this + s.roles.
func (s siteVersionRole) Roles() roles.Roles {
	return s.roles
}

func (s siteVersionRole) Version() versions.Version {
	return s.version
}

// TODO1 remove this + s.versions.
func (s siteVersionRole) Versions() versions.Versions {
	return s.versions
}
...
func NewHugoSites(cfg deps.DepsCfg) (*HugoSites, error) {
	...
	for i, v1 := range sitesVersionsRoles {
		for j, v2 := range v1 {
			for k := range v2 {
				...
				vrs.role = roles.NewRoleSite(rolesSorted[k], vrs)
				vrs.versionInternal = versionsSorted[j]
				vrs.version = versions.NewVersionSite(versionsSorted[j], vrs)
				...
				vrs.pageMap = newPageMap(i, j, k, vrs, memCache, pageTrees)
	...
	for i, v1 := range sitesVersionsRoles {
		for j, v2 := range v1 {
			roles := siteVersionRoles[types.Ints2{i, j}]
			for k, s := range v2 {
				s.versions = siteRoleVersions[types.Ints2{i, k}]
				s.roles = roles
			}
		}
	}
...
func newHugoSites(
	...
	sitesVersionsRolesMap := map[sitematrix.Vector]*Site{}
	for _, v1 := range sitesVersionsRoles {
		for _, v2 := range v1 {
			for _, s := range v2 {
				sitesVersionsRolesMap[s.dims] = s
			}
		}
	}
...
// Valid dimension names are those defined in sitematrix.DimensionLanguage, sitematrix.DimensionVersion, and sitematrix.DimensionRole.
func (s *Site) Dimension(d string) page.SiteDimension {
	switch d {
	case sitematrix.Language.Name():
		return s.language
	case sitematrix.Version.Name():
		return s.version
	case sitematrix.Role.Name():
		return s.role
	default:
