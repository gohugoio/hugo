...
type Site struct {
	...
	*deps.Deps `json:"-"`
	...
	// Shortcut to the home page. Note that this may be nill if
...
func (s *Site) initWrap(init *lazy.Init) func() {
	return func() {
		s.initInit(init)
	}
}

func (s *Site) initInit(init *lazy.Init) {
	_, err := init.Do()
	if err != nil {
		s.h.FatalError(err)
...
func (s *Site) prepareInits() {
	...
	s.init.prevNextInSection = init.Branch(func() (interface{}, error) {
		for _, p1 := range s.workAllPages {
			// TODO(bep) page check logic vs root section
...
// TODO(bep) page consolidate all site stats into this
...
func (s *Site) reset() *Site {
	return &Site{Deps: s.Deps,
		...
		buildStats:             &buildStats{},
		init:                   s.init, // TODO(bep) page Reset
		PageCollections:        newPageCollections(),
		siteConfigHolder:       s.siteConfigHolder,
	}
...
func (s *siteRefLinker) refLink(ref string, source interface{}, relative bool, outputFormat string) (string, error) {
	...
	if refURL.Fragment != "" {
		...
		/*if refURL.Path != "" && target != nil && !top(target).getRenderingConfig().PlainIDAnchors {
			link = link + ":" + target.File().UniqueID()
		} else if p != nil && !top(p).getRenderingConfig().PlainIDAnchors {
...
func (s *Site) setupSitePages() {
	...
	for _, page := range regularPages {
		// TODO(bep) page
		/*		pagep := top(page)
				if i > 0 {
					pagep.NextPage = regularPages[i-1]
				}

				if i < len(regularPages)-1 {
					pagep.PrevPage = regularPages[i+1]
				}
		*/
...
// TODO(bep) page check if we can avoid adding these to the interface
...
func (s *SiteInfo) SitemapAbsURL() string {
	...
	p += s.s.siteConfigHolder.sitemap.Filename
	return p
}
...
func (s *Site) getLanguageTargetPathLang() string {
	if s.h.IsMultihost() {
...
func (s *Site) getLanguagePermalinkLang() string {
...
func (s *Site) assembleTaxonomies() error {
	defer s.timerStep("assemble Taxonomies")

	s.Taxonomies = make(TaxonomyList)
	taxonomies := s.siteConfigHolder.taxonomiesConfig
	...
	for singular, plural := range taxonomies {
		...
		// TODO(bep) page raw vs
		for _, p := range s.workAllPages {
			vals := getParam(p, plural, !s.Info.preserveTaxonomyNames)
			...
			if vals != nil {
				if v, ok := vals.([]string); ok {
					for _, idx := range v {
						x := page.WeightedPage{Weight: weight, Page: p}
...
// TODO(bep) page move
func permalinkForOutputFormat(ps *helpers.PathSpec, link string, f output.Format) (string, error) {
	var (
		baseURL string
		err     error
	)
	...
	if f.Protocol != "" {
		baseURL, err = ps.BaseURL.WithProtocol(f.Protocol)
		if err != nil {
			return "", err
		}
	} else {
		baseURL = ps.BaseURL.String()
	}
	return ps.PermalinkForBaseURL(link, baseURL), nil
}
...
// TODO(bep) page consider this vs the header
...
func (s *Site) renderForLayouts(name string, d interface{}, w io.Writer, layouts ...string) (err error) {
	...
	if templ == nil {
		...
		if p, ok := d.(*pageState); ok {
			log.Printf("Found no layout file for output format %q on %q: %s\n", p.currentOutputFormat.Name, p.Kind(), errMsg)
		} else {
