...
func (s *SiteInfo) SourceRelativeLink(ref string, currentPage *Page, reusedPage *Page) (string, error) {
	...
	if refURL.Path != "" {
		...
		if strings.IndexRune(refPath, os.PathSeparator) == 0 { // filepath.IsAbs fails to me.
			...
			if currentPage != nil {
				if reusedPage != nil && reusedPage.LogicalName() != "index.md" &&
...
func (s *SiteInfo) LookupPage(ref string, currentPage *Page) (*Page, error) {
	...
	if refURL.Path != "" {
		...
		target = searchPage(refPath, s.PageSnippets, true)
		if target == nil {
			target = searchPage(refPath, s.AllRegularPages, false)
		}
		if target == nil {
...
func searchPage(refPath string, pagesToSearch Pages, snippetSearch bool) *Page {
	var target *Page
	...
	if snippetSearch {
		snippetPath := strings.TrimSuffix(refPath, string(os.PathSeparator)) + ".snippet.md"
		for _, page := range pagesToSearch {
			if page.Source.Path() == snippetPath {
				target = page
				break
			}
		}
...
func readSourceFile(s *Site, file *source.File, results chan<- HandledResult) {
	h := NewFilenameMetaHandler(file.Extension(), file.LogicalName())
	if h != nil {
...
func pageConverter(pages <-chan *Page, results HandleResults, wg *sync.WaitGroup) {
	...
	for page := range pages {
		var h *MetaHandle
		if page.Markup != "" {
			h = NewFilenameMetaHandler(page.Markup, page.File.LogicalName())
		} else {
			h = NewFilenameMetaHandler(page.File.Extension(), page.File.LogicalName())
		}
		if h != nil {
...
func readCollator(s *Site, results <-chan HandledResult, errs chan<- error) {
	...
	for r := range results {
		...
		if r.page != nil {
			s.addPage(r.page)
		} else if r.snippet != nil {
			s.addPageSnippet(r.snippet)
		} else {
