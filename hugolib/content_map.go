...
type resourceSource struct {
	dims   sitesmatrix.VectorProvider
	path   *paths.Path
...
func (p *resourceSource) matchDirectOrInDelegees(vec sitesmatrix.Vector) (contentNode, sitesmatrix.Vector) {
	// TODO1
	return p, p.dims.FirstVector()
}
		...
		contentAdapter = pagesfromdata.NewPagesFromTemplate(
			pagesfromdata.PagesFromTemplateOptions{
				...
				HandleResource: func(pt *pagesfromdata.PagesFromTemplate, rc *pagemeta.ResourceConfig) error {
					...
					rs := &resourceSource{path: rc.PathInfo, rc: rc, opener: nil, fi: pt.GoTmplFi, dims: s.siteVector}
