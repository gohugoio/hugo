...
func doTestMultiSitesBuild(t *testing.T, configTemplate, configSuffix string) {
	...
	getPageDoc1En, _ := enSite.getPage(filepath.ToSlash(doc1en.Path()))
	getPageDoc1EnBase, _ := enSite.getPage("sect/doc1")
	getPageDoc1Fr, _ := frSite.getPage(filepath.ToSlash(doc1fr.Path()))
	getPageDoc1FrBase, _ := frSite.getPage("sect/doc1")
	...
	// Check bundles
	bundleFr, _ := frSite.getPage("bundles/b1/index.md")
	...
	bundleEn, _ := enSite.getPage("bundles/b1/index.en.md")
