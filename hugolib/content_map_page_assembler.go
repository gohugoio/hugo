...
type createPagesHandler struct {
	m *pageMap
...
func (sa *sitePagesAssembler) createMissingTaxonomies() error {
	...
	for _, s := range sa.s.h.sitesLanguages {
		...
		hdebug.Printf("site %q: %v", s.Lang(), s.siteVector.Language())
	...
	for viewName, languages := range viewLanguages {
		...
		matrixAllForLanguages := sitesmatrix.NewIntSetsBuilder(sa.s.h.Conf.ConfiguredDimensions()).WithLanguageIndex(languages...).WithAllIfNotSet().Build()
