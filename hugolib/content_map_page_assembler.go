...
func (a *allPagesAssembler) createAllPages() error {
	...
	transformPages2 := func(s string, n contentNode, cascades *page.PageMatcherParamsConfigs) (n2 contentNode, ns doctree.NodeTransformState, err error) {
		defer func() {
			if n2 != nil {
		...
		case *pageMetaSource:
			is := make(contentNodes[contentNodePage])
			_, err := handlePageMetaSource(v, is, false)
			if err != nil {
				return nil, 0, fmt.Errorf("failed to create page from pageMetaSource %s: %w", s, err)
			}
			return is, ns, nil
	...
	transformPages := func(s string, n contentNode, cascades *page.PageMatcherParamsConfigs) (n2 contentNode, ns doctree.NodeTransformState, err error) {
		...
		n2, ns, err = transformPages2(s, n, cascades)
		return
	}
