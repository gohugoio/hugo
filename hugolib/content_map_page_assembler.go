...
func (a *allPagesAssembler) createAllPages() error {
	...
	defer func() {
		...
		for _, s := range a.droppedTerms {
			a.pw.Tree.Delete(s)
			a.rw.Tree.DeletePrefix(paths.AddTrailingSlash(s))
		}
	}()
	...
	type (
		transformFuncWithCascades func(s string, n contentNode, cascades *page.PageMatcherParamsConfigs) (n2 contentNode, ns doctree.NodeTransformState, err error)
	)
	...
	doTransformPages := func(s string, n contentNode, cascades *page.PageMatcherParamsConfigs) (n2 contentNode, ns doctree.NodeTransformState, err error) {
		...
		var handlePageMetaSource func(v contentNode, n contentNodes[contentNodePage], replaceVector bool) (n2 contentNode, err error)
		handlePageMetaSource = func(v contentNode, n contentNodes[contentNodePage], replaceVector bool) (n2 contentNode, err error) {
			if n != nil {
				n2 = n
			}
			...
			case *pageMetaSource:
				...
				sitesMatrix.ForEeachVector(func(vec sitesmatrix.Vector) bool {
					...
					if err = p.setMetaPost(ms.cascadeCompiled, cascades); err != nil {
						return true
					}
					...
					if !drop && n == nil {
						if n2 == nil {
							...
							// Upgrade to a map.
							n = make(contentNodes[contentNodePage])
							ps := n2.(*pageState)
			...
			case contentNodes[contentNodePage]:
				for _, vv := range ms {
					...
					if m, ok := n2.(contentNodes[contentNodePage]); ok {
						n = m
					}
		...
		case contentNodes[contentNodePage]:
			...
			for i, vv := range v {
				...
				case *pageMeta:
					...
					if err != nil {
						return nil, 0, fmt.Errorf("failed to create page %s: %w", s, err)
					}
				case *pageMetaSource:
	...
	// TODO1 move this and also use this in Shift/Insert.
		...
		func() error {
			for k, v := range a.seenTerms {
				...
				if len(v) > 0 {
					...
					// TODO1 check usage (panic)
					switch ns {
					case doctree.NodeTransformStateReplaced:
						a.pw.Tree.InsertRaw(termKey, n2)
					case doctree.NodeTransformStateDeleted:
						a.droppedTerms = append(a.droppedTerms, termKey)
					}
...
func (sa *sitePagesAssembler) removeShouldNotBuild() error {
	// TODO1 check if this is still needed. See craateAllPages.
	if true {
		// return nil
	}
	s := sa.s
