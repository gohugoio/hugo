...
func (s *Site) newPageFromPageMeta(m *pageMeta) (*pageState, error) {
	p, err := s.doNewPageFromPageMeta(m)
	if err != nil {
...
func (s *Site) doNewPageFromPageMeta(m *pageMeta) (*pageState, error) {
	...
	ps := &pageState{
		...
		pageCommon: &pageCommon{
			store:                      sync.OnceValue(func() *maps.Scratch { return maps.NewScratch() }), // Rarely used.
