...
func (s *contentNodeShifter) shiftContentNodeMap(v contentNodeMap, siteVector sitesmatrix.Vector, fallback bool) (contentNode, bool) {
...
func (s *contentNodeShifter) shiftResourceSourcesSlice(v resourceSourcesSlice, siteVector sitesmatrix.Vector, fallback bool) (contentNode, bool) {
	iter := func(yield func(n contentNode) bool) {
		for _, vv := range v {
			if !yield(vv) {
				return
...
func (s *contentNodeShifter) Shift(n contentNode, siteVector sitesmatrix.Vector, fallback bool) (contentNode, bool) {
	switch v := n.(type) {
	case contentNodeMap:
		return s.shiftContentNodeMap(v, siteVector, fallback)
	case resourceSources: // TODO1 remove this type.
		panic("TODO1 remove me")
	case resourceSourcesSlice:
		return s.shiftResourceSourcesSlice(v, siteVector, fallback)
	case *resourceSource:
		if v.matchSiteVector(siteVector) {
			return v, true
		}
	case *pageState:
...
func (s *contentNodeShifter) Insert(old, new contentNode) (contentNode, contentNode, bool) {
	...
	case *pageState:
		switch new := new.(type) {
		case *pageState:
			if vv.s.siteVector == new.s.siteVector {
				if new != old {
