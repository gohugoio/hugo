...
func (ps *pageMetaSource) isEmpty() bool {
	return false
}
...
type pageMetaSource struct {
	...
	// TODO1 need?
	termS     string // Set for kind == KindTerm.
	singularS string // Set for kind == KindTerm and kind == KindTaxonomy.
	...
	// These are only set in watch mode. TODO1 remove me.
...
func (s *Site) newPageFromPageMetasource(ms *pageMetaSource) (*pageState, error) {
	...
	return s.newPageFromPageMeta(m)
}
...
// Prepare for a rebuild of the data passed in from front matter. TODO1
...
func (ps *pageState) setMetaPost(cascadesSelf, cascadesIncoming *page.PageMatcherParamsConfigs) error {
	...
	allCascades := hiter.Concat(cascadesSelf.All(), cascadesIncoming.All())
