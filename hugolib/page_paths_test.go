	...
	for _, langPrefix := range []string{"", "no"} {
		for _, trimTrailingSlash := range []bool{false, true} {
			for _, uglyURLs := range []bool{false, true} {
				t.Run(fmt.Sprintf("langPrefix=%q,trimTrailingSlash=%t,uglyURLs=%t", langPrefix, trimTrailingSlash, uglyURLs),
					func(t *testing.T) {
						...
						for i, test := range tests {
							...
							test.d.TrimTrailingSlash = trimTrailingSlash
							test.d.UglyURLs = uglyURLs
							test.d.LangPrefix = langPrefix
							test.d.Dir = filepath.FromSlash(test.d.Dir)
							isUgly := (uglyURLs || trimTrailingSlash) && !test.d.Type.NoUgly
							...
							} else if !strings.HasPrefix(expected, "/index") || test.d.Addends != "" {
								if (isUgly && test.d.URL == "") || (trimTrailingSlash && test.d.URL != "") {
