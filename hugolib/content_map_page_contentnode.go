...
type (
	...
	contentNodeVariantAdder interface {
		addContentNodeVariant(sitesmatrix.Vector) contentNode // TODO1 check if used.
	}
...
func (h helperContentNode) toSeq(ns ...contentNode) contentNodeSeq {
	if len(ns) == 1 {
		if n, ok := ns[0].(contentNodeSeq); ok {
			return n
		}
	}
	return func(yield func(contentNode) bool) {
		for _, n := range ns {
			if !n.forEeachContentNode(func(_ sitesmatrix.Vector, n contentNode) bool {
				return yield(n)
			}) {
				return
			}
