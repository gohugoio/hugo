...
type (
	Config[T any] struct {
		// MatrixRow configures a row in the matrix (e.g. [role, language]).
		// It cannot be changed once set.
		MatrixRow Dimensions
	...
	Shifter[T any] interface {
		...
		Delete(v T, dimension []int) (bool, bool)

		// Shift shifts T into the given dimension.
		// It may return a zero value and false.
		// If exact is set, it will only return an exact match.
		Shift(v T, dimension []int, exact bool) (T, bool)
...
func (r *NodeShiftTree[T]) LongestPrefix(s string, exact bool, predicate func(v T) bool) (string, T) {
	for {
		...
		if found {
			if t, ok := r.shift(v.(T), exact); ok && (predicate == nil || predicate(t)) {
...
func (r *NodeShiftTree[T]) GetAll(s string) []T {
	s = cleanKey(s)
	v, ok := r.tree.Get(s)
	if !ok {
		return nil
...
func (r *NodeShiftTree[T]) GetDimension(s string, d int) []T {
	s = cleanKey(s)
	v, ok := r.tree.Get(s)
	if !ok {
		return nil
...
type NodeShiftTreeWalker[T any] struct {
	...
	Handle func(string, T) (terminate bool, err error)
...
func (r *NodeShiftTreeWalker[T]) toT(tree *NodeShiftTree[T], v any) (t T, ok bool) {
	if r.NoShift {
		t = v.(T)
		ok = true
	} else {
		t, ok = tree.shift(v.(T), r.Exact)
	}
	return
}
...
func (t NodeShiftTree[T]) clone() *NodeShiftTree[T] {
	dimensions := make(Dimensions, len(t.dims))
	copy(dimensions, t.dims)
	t.dims = dimensions

	return &t
}

func (r *NodeShiftTree[T]) shift(t T, exact bool) (T, bool) {
