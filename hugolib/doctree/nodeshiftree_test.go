...
func (s *testShifter) Shift(n *testValue, dimension sitesmatrix.Vector, exact, matchDelegees bool) (*testValue, bool, sitesmatrix.Dimension) {
	if s.echo {
	...
	if n.NoCopy {
		...
		return nil, false, sitesmatrix.Language
	}
	c := *n
	c.Lang = dimension[0]
	return &c, true, sitesmatrix.Language
}
