...
You can always add non-typical front-matter to any piece of content, but since it takes extra work to develop a theme that accounts for unique data, consistency is simpler.
...
3. **Focus on more important things**. Avoid having to remember all of the fields that needs to be associated with each piece of content. (This is particularly important for larger projects with complex front-matter and a variety of content types.)
...
This `hugo new` command creates a new content file inside the project's `content` directory — in this case, a file named `hello-world.md` — and if you open this file, you'll notice that it contains the following front-matter:
...
To achieve this, create a file named `default.md` inside the `archetypes` folder of a Hugo project. (If the folder doesn't exist, simply create it.)
...
You'll also notice that we're writing the front-matter in the TOML format. It is possible to define archetype front-matter in other formats, but a setting needs to be changed in the configuration file. See the "[Archetype Formats](#archetype-formats)" section of this article for more details.

Next, run the following command inside the command line:
...
This functionality, however, can be overwritten.
...
* Prior to Hugo v0.13, some users received [an "EOF" error when using archetypes](https://github.com/spf13/hugo/issues/776), related to what text editor they used to create the archetype. As of Hugo v0.13, this error was [resolved](https://github.com/spf13/hugo/pull/785).