...
* AsciiDoc's table of contents, generated when the `:toc:` attribute is set,
  may not be rendered when the `--no-header-footer` setting is used. It affects
  `asciidoc` and older versions of `asciidoctor`. As a workaround ensure that
  the `asciidoctor` executable with version >= 1.5.3 is installed and found in
  the `PATH` {{< gh 1685 >}}.
