...
* AsciiDoc's table of contents, generated when the `:toc:` attribute is set,
  may not be rendered. It affects `asciidoc` and older versions of
  `asciidoctor` and is due to the fact that they are executed with the flag
  `--no-header-footer`. As a workaround ensure that the `asciidoctor`
  executable with version >= 1.5.3 is installed and found in the `PATH`
  {{< gh 1685 >}}.
