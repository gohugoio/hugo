...
func TestInit(t *testing.T) {
	...
	f1 := func(name string) func() (interface{}, error) {
		return func() (interface{}, error) {
			result += name + "|"
			size := bigOrSmall()
			strings.Repeat("Hugo Rocks! ", size)
			return name, nil
		}
	}
	...
	branch_1 := root.Branch(f1("branch_1"))
	branch_1.Add(f1("branch_1_1"))
	branch_1_2 := branch_1.Add(f1("branch_1_2"))
	branch_1_2_1 := branch_1_2.Add(f1("branch_1_2_1"))
	...
	for i := 0; i < 100; i++ {
		...
		go func(i int) {
			...
			if rand.Intn(10) > 5 {
				branch := branch_1_2.Branch(f2())
				init := branch.Add(f2())
				_, err = init.Do()
				assert.NoError(err)
			} else {
				_, err = branch_1_2_1.Do()
				assert.NoError(err)
			}
			_, err = branch_1_2.Do()
			assert.NoError(err)
