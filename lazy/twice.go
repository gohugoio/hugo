...
// twice is like sync.Once, but it can be reset so the action can be repeated if needed.
type twice struct {
	mu   sync.Mutex
	lock uint32
	done uint32
...
func (t *twice) InProgress() bool {
	return atomic.LoadUint32(&t.lock) == 1
}

func (t *twice) Done() bool {
	return atomic.LoadUint32(&t.done) == 1
}

func (t *twice) ResetWithLock() *sync.Mutex {
	t.mu.Lock()
