...
{{ end }}`)
	t.addInternalShortcode("peertube.html", `{{ if .IsNamedParams }}<div{{ with .Get "class" }} class="{{ . }}"{{ end }}><iframe allowfullscreen="allowfullscreen" frameborder="0" width="512" height="315" src="{{ cond (hasPrefix (.Get "server") "http") (.Get "server") (printf "//%s" (.Get "server")) }}/videos/embed/{{ .Get "id" }}"></iframe></div>{{ else }}<div><iframe allowfullscreen="allowfullscreen" frameborder="0" width="512" height="315" src="{{ cond (eq (len .Params) 1) (replace (.Get 0) "/videos/watch/" "/videos/embed/") (printf "//%s/videos/embed/%s" (.Get 0) (.Get 1)) }}"></iframe></div>{{ end }}`)
