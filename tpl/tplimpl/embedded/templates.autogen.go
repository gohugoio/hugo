...
{{- else -}}
 ...
 <div {{ with $class }}class="{{ . }}"{{ else }}style="position: relative; padding-bottom: 56.25%; padding-top: 0px; height: 0; overflow: hidden;"{{ end }}>
   <iframe src="//player.vimeo.com/video/{{ $id }}" {{ if not $class }}style="position: absolute; top: 0; left: 0; width: 100%; height: 100%; border:0;" {{ end }}title="Vimeo Video" webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe>
 </div>
{{- end -}}
{{- end -}}`},
