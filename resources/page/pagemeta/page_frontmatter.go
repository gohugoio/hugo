...
type PageConfig struct {
	...
	PageConfigLate  `mapstructure:",squash"`
}
...
type PageConfigEarly struct {
	...
	Frontmatter maps.Params `mapstructure:"-" json:"-"` // The original front matter.

	// Compiled values.
	SitesMatrixFallbacks `mapstructure:"-" json:"-"`
...
func (p *SitesMatrixFallbacks) MatchSiteVector(siteVector sitesmatrix.Vector) bool {
...
func (f *frontmatterFieldHandlers) newDateFilenameHandler(setter func(d *FrontMatterDescriptor, t time.Time)) frontMatterFieldHandler {
	return func(d *FrontMatterDescriptor) (bool, error) {
		...
		if _, found := d.PageConfigLate.Params["slug"]; !found {
