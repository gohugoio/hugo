...
type PageConfigEarly struct {
	...
	// Compiled values.
	SitesMatrixAndFallbacks `mapstructure:"-" json:"-"`
...
type SitesMatrixAndFallbacks struct {
	SitesMatrix    sitesmatrix.VectorStore `mapstructure:"-" json:"-"`
	SitesFallbacks sitesmatrix.VectorStore `mapstructure:"-" json:"-"`
}

func (p PageConfigLate) shallowCloneForSite() *PageConfigLate {
	// TODO1 not so shallow.
	return &p
}

func (p *SitesMatrixAndFallbacks) MatchSiteVector(siteVector sitesmatrix.Vector) bool {
	return p.SitesMatrix.HasAnyVector(siteVector)
}

func (p *SitesMatrixAndFallbacks) MatchLanguageOrLanguageFallback(siteVector sitesmatrix.Vector) bool {
	i := siteVector.Language()
	return p.SitesMatrix.HasLanguage(i) || p.SitesFallbacks.HasLanguage(i)
}

func (p *SitesMatrixAndFallbacks) MatchRoleOrRoleFallback(siteVector sitesmatrix.Vector) bool {
	i := siteVector.Role()
	return p.SitesMatrix.HasRole(i) || p.SitesFallbacks.HasRole(i)
}

func (p *SitesMatrixAndFallbacks) MatchVersionOrVersionFallback(siteVector sitesmatrix.Vector) bool {
	i := siteVector.Version()
	return p.SitesMatrix.HasVersion(i) || p.SitesFallbacks.HasVersion(i)
}

func ClonePageConfigForRebuild(pe *PageConfigEarly, pl *PageConfigLate, params map[string]any) *PageConfigLate {
	pp := &PageConfigLate{}
	if pe.IsFromContentAdapter {
		// TODO1 pp.ContentAdapterData = params
	} else {
		pp.Params = params
	}
	return pp
...
func ClonePageConfigForSite(p *PageConfigLate) *PageConfigLate {
...
func (p *PageConfigEarly) CompileEarly(pi *paths.Path, cascades *page.PageMatcherParamsConfigs,
	...
	for cascade := range cascades.All() {
		...
		// TODO1 make sure Kind is set here.
