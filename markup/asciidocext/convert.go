...
// SupportsDitaaDiagrams reports whether the AsciiDoc converter can render
// Ditaa diagrams. Only used in tests.
func SupportsDitaaDiagrams() (bool, error) {
	supportsDitaaDiagrams, err := internal.CanRenderDitaaDiagrams()
	if htesting.SupportsAll() {
		if !supportsDitaaDiagrams {
			panic(err)
		}
		return true, nil
	}

	return supportsDitaaDiagrams, err
}
