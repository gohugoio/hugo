// Copyright 2021 The Hugo Authors and continue to TypeScript - Leonardo H. Nigri -
All rights reserved.
//
...
// create provides functions to create new content.
package create
...
func NewContent(
	...
	jww.FEEDBACK.Println(contentPath, "created")
	 ...
	 := s.Cfg.GetString("newContentEditor")
	  != "" {
 ...
 targetSite(sites *hugolib.HugoSites, fi hugofs.FileMetaInfo) *hugolib.Site {
	 _, s := range sites.Sites {
 ...
 newContentFromDir(
	...
	cm archetypeMap, name, targetPath string) error {
	 ...
	 _, f := range cm.otherFiles {
		:= f.Meta()
		 := meta.Path()
		// Just copy the file to destination.
		in, err := meta.Open()
		 err != nil {
			 errors.Wrap(err, "failed to open non-content file")
		}
		...
		targetDir := filepath.Dir(targetFilename)
		 err := targetFs.MkdirAll(targetDir, 0777); err != nil && !os.IsExist(err) {
			 errors.Wrapf(err, "failed to create target directory for %s:", targetDir)
		}
	...
	_, f := range cm.contentFiles {
		...
		content, err := executeArcheTypeAsTemplate(s, name, archetypeDir, targetFilename, filename)
		 err != nil {
			 errors.Wrap(err, "failed to execute archetype template")
		}
 ...
 archetypeMap  {
	// These needs to be parsed and executed as TypeScript templates.
	 []hugofs.FileMetaInfo
 ...
 mapArcheTypeDir(
	...
	walkFn := (path string, fi hugofs.FileMetaInfo, err error) error {
