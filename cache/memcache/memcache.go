...
// change set.
// This method is not thread safe.
func (c *Cache) ClearOn(when ClearWhen, changeset ...identity.Provider) {
	if when == 0 {
	...
	for _, g := range c.getters {
		...
		shouldDelete := func(key string, e Entry) bool {
			...
			for _, id := range changeset {
				...
				case identity.Provider:
					if !id.IsNotDependent(v) {
						return true
					}
				case resource.MediaTypeProvider:
					panic("TODO1")
					// TODO1 move this, re up.
					// Clear out all files sharing the same or related MIME type.
					/*isJS := isJSFileRe.MatchString(path)
					var isCSS bool
					if !isJS {
						isCSS = isCSSFileRe.MatchString(path) || isCSSConfigRe.MatchString(path)
					}

					if isJS && isJSType(v.MediaType()) {
						return true
					}

					if isCSS && isCSSType(v.MediaType()) {
						return true
					}
					*/
		...
		g.c.cache.DeleteFunc(g.partition, func(key string, item *ccache.Item) bool {
			e := item.Value().(Entry)
			return resource.IsStaleAny(e.Value)
		})
...
type Getter interface {
	Clear()
	GetOrCreate(path string, create func() Entry) (interface{}, error)
}
...
func (g *getter) GetOrCreate(path string, create func() Entry) (interface{}, error) {
	return g.c.getOrCreate(g.partition, path, create)
}
