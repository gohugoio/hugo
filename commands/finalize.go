...
var finalizeCmd = &cobra.Command{
	Use:   "finalize path/to/content",
	Short: "Finalize changes the content's draft status from 'True' to 'False'",
	Long:  `Finalize changes the content's draft status from 'True' to 'False' and updates the date to the current date and time. If the content's draft status is 'False', nothing is done`,
	Run:   Finalize,
}
...
func Finalize(cmd *cobra.Command, args []string) {
	...
	w, err := finalizeContent(p)
	if err != nil {
		jww.ERROR.Printf("an error occurred while publishing %q: %s", location, err)
		return
	}

	f, err = os.OpenFile(location, os.O_WRONLY|os.O_TRUNC, 0644)
	if err != nil {
		jww.ERROR.Printf("%q not published due to error opening file to save changes: %q\n", location, err)
		return
	}
	defer f.Close()
	_, err = w.WriteTo(f)
	if err != nil {
		jww.ERROR.Printf("%q not published due to save error: %q\n", location, err)
	}
	return
}

// finalizeContent: if the content is a draft, change it's draft status to
// 'false' and set the date to time.Now(). If the draft status is already
// 'false', don't do anything.
func finalizeContent(p parser.Page) (bytes.Buffer, error) {
