...
var componentFsContext = struct {
	IsInLeafBundle hcontext.ContextDispatcher[bool]
}{
	IsInLeafBundle: hcontext.NewContextDispatcher[bool](contextKeyIsInLeafBundle),
}
...
func (f *componentFsDir) ReadDirWithContext(ctx context.Context, count int) ([]iofs.DirEntry, context.Context, error) {
	...
	sort.Slice(fis, func(i, j int) bool {
		...
		if bi == bj {
			...
			if l1 != l2 {
				// Pull the ones with the least number of vectors to the top.
				// TODO1 we may need to get smarter.
				return l1 < l2
			}
		}
